...
//TODO: Better error handling, when would this happen?
	...
	int n = g.nodeNum();
	// Abbildung von den Kanten auf Zahlen, Zahlen=Spalten
  ...
  for(m=0;m<(n*(n-1))/2;m++){
	...
	// Erstelle zusätzliche spalten für die Y Variablen
	if(_useKCluster){
	...
	std::list<int> weights; // Die Summe wird später addiert (Objektfunktion)
	...
	// Für alle uvw werden hier die ungleichungen erstellt und in das Build Objekt geschrieben
	for (FullGraph::NodeIt i(g);i != INVALID ;++i) {
			FullGraph::NodeIt j(g); j = i;
			for (++j,y=x+1; j != INVALID, y<n; ++j, y++) {
					...
					for (++k, z=y+1; k != INVALID, z<n; ++k,z++) {
						...
						// Ist die Zeile notwendig/korrekt?
			...
			if(_minCluster > 1 || _maxCluster < INT_MAX){
				...
				bo.addRow(n-1,neighbors,newEle,_minCluster-1,_maxCluster-1);
			}
			x++;
	}
  ...
  for(m=0,ptr = weights.begin();m<(n*n-1)/2,ptr != weights.end();m++,ptr++){
	...
	// Die Y-var müssen auch noch auf 0 oder 1 gesetzt werden
	if(_useKCluster){
  ...
  si.findIntegers(false);
	...
	// Ausgaben die für die Tests gebraucht wurden
	//ofstream output;
